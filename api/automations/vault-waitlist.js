...
export default async function handler(request, response) {
  ...
  const groupId = process.env.MAILERLITE_VAULT_WAITLIST_GROUP_ID;

  if (!apiToken || !groupId) {
    console.error('MailerLite waitlist missing environment variables');
    return sendJson(response, 500, { success: false, error: 'I couldn’t add you yet. Please try again.' });
  }
  ...
  try {
    ...
    if (!mailerliteResponse.ok) {
      console.error('MailerLite waitlist failed', {
        status: mailerliteResponse.status,
        emailDomain: normalized.email.split('@')[1] || 'unknown',
      });
      return sendJson(response, 502, { success: false, error: 'I couldn’t add you yet. Please try again.' });
    }

    return sendJson(response, 200, { success: true });
  } catch (error) {
    console.error('MailerLite waitlist request error', {
